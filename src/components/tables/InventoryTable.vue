<template>
  <v-row no-gutters>
    <v-col cols="12" md="4">
      <!-- Input Search -->
      <SearchInput label="Buscar producto" @search="handleSearch" />
      <!-- End Input Search -->
    </v-col>
    <v-spacer />
...
// Interface
import type { DataTableHeader, InventoryData } from '@/interfaces'
// Props
const props = defineProps({
  headers: Array as () => DeepReadonly<DataTableHeader[] | DataTableHeader[][]>,
  items: Array as () => InventoryData[]
})
// Const
const dialog = ref<boolean>(false)
const dialogDelete = ref<boolean>(false)
const tableInventory = ref<HTMLElement | null>(null)
const search = ref<string>('')
const data = ref<InventoryData[]>([])
const editedIndex = ref<number>(-1)
const editedItem = ref<InventoryData>({
  product: '',
  stock: 0,
  shopping: 0,
  sales: 0
})
const defaultItem = ref<InventoryData>({
  product: '',
  stock: 0,
  shopping: 0,
  sales: 0
...
const formTitle = computed(() => {
  return !editedItem.value.id ? 'Nuevo Producto' : 'Editar Producto'
})

const editItem = (item: InventoryData) => {
...
const deleteItem = (item: InventoryData) => {
...
const save = async () => {
  let { id, stock, sales, shopping, ...res } = editedItem.value
  stock = +stock
  sales = +sales
  shopping = +shopping
  try {
    if (!id) {
      ...
      showSnackbar.value = true
      message.value = `¡El producto ${res.product} fue agregado con exito!`
      color.value = 'deepteal600'
      close()
    } else {
      // await user.updateUser(id, { cc, phone, ...res })
      showSnackbar.value = true
      message.value = `¡El producto ${res.product} fue actualizado con exito!`
      color.value = 'deepteal600'
      close()
    }
  } catch (error: any) {
